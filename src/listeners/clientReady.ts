...
import {
	checkIsReadyForDeadlineAnnouncement,
	create_deadline_announcement,
} from "../shared/property-deadlines";
...
function setStatus(client: Client, container: Container) {
	if (NODE_ENV === "production") {
		client.user?.setActivity("out for illegal business operations", {
			type: ActivityType.Watching,
		});
		client.user?.setStatus("idle");
	} else {
		client.user?.setActivity(
			"under maintenance, please expect degraded performance",
			{ type: ActivityType.Custom },
		);
		client.user?.setStatus("dnd");

		const guilds: string = client.guilds.cache
...
async function checkWeeklyMonday() {
	...
	try {
		...
		console.log(
			"No previous check found or error reading file, proceeding with check.",
		);
		console.warn(error);
		// return true;
	}
...
async function logWeek(usersMessaged: User[]) {
	...
	const logEntry = {
		...
		usersMessaged: usersMessaged.map((user) => ({
			id: user.id,
			tag: user.tag,
		})),
	};
...
async function runWeeklyCheck(client: Client, container: Container) {
	...
	for (const guild of client.guilds.cache.values()) {
		...
		const membersWithRoles = guild.members.cache.filter((member) =>
			member.roles.cache.some((role) =>
				SEARCH_ROLE_IDS.includes(role.id),
			),
		);
	...
	const uniqueUsersToMessage = Array.from(
		new Set(usersToMessage.map((user) => user.id)),
	...
	if (backlog_size === 0) {
		container.logger.info(
			"No open requests in backlog, skipping weekly check messaging.",
		);
		return;
	}

	container.logger.info(
		`Messaging ${uniqueUsersToMessage.length} users about the backlog of ${backlog_size} requests.`,
	);
		...
		.addTextDisplayComponents((textDisplay) =>
			textDisplay.setContent(
				templateMessage
					.trim()
					.replace(
						"FORMAT_NUMBER_OF_REQUESTS",
						backlog_size.toString(),
					)
					.replace(
						"FORMAT_LINK_TO_CHANNEL",
		...
		.addActionRowComponents((actionRow) =>
			actionRow.addComponents(
				new ButtonBuilder()
					...
					.setURL(
						`https://discord.com/channels/962005830960562216/1433519872209322196`,
					),
				new ButtonBuilder()
					.setLabel("View Text-Based Requests")
					.setStyle(5) // Link style
					.setURL(
						`https://discord.com/channels/735894836577697913/1096981698052370532`,
					),
				new ButtonBuilder()
		...
		try {
			...
			if (!dmChannel || !dmChannel.isSendable()) {
				container.logger.warn(
					`Cannot send DM to ${user.tag}, skipping.`,
				);
				continue;
			}
			...
			container.logger.error(
				`Failed to send message to ${user.tag}:`,
				error,
			);
...
export class ClientReadyListener extends Listener {
	public run(client: Client) {
		...
		this.container.logger.info(`Ready! Logged in as ${tag}`);
			...
			try {
				...
				if (isReady) {
					...
					this.container.logger.info(
						"Deadline announcement created successfully.",
					);
				}
			} catch (error) {
				this.container.logger.error(
					"Error during deadline announcement check:",
					error,
				);
				Sentry.captureException(error);
			}
