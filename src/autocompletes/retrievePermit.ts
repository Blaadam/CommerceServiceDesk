...
async function retrieveAllPermitCards(span?: Sentry.Span) {
	...
	span?.setAttribute(
		"cache.isCacheValid",
		cachedCards !== null && now - lastFetchTime < CACHE_TTL,
	);
	span?.setAttribute(
		"cache.cachedCardsCount",
	...
	span?.setAttribute(
		"trello.cardNameExclusions",
		CARD_NAME_EXCLUSIONS.join(","),
	);
	...
	try {
		...
		span?.setAttribute(
			"cache.newCachedCardsCount",
			cachedCards?.length || -1,
		);
...
export default async function retrievePermit(
	interaction: AutocompleteCommand | any,
	span?: Sentry.Span,
) {
	...
	const options: [{ name: string; shortUrl: string }] = cards.map(
