...
async function GetManagersFromDistrict(
	district: string,
	span?: any,
): Promise<string[]> {
	...
	span?.setAttribute(
		"database.query",
		`findMany where District = ${district}`,
	);
...
export default class ViewHistoryCommand extends Command {
	...
	public async chatInputRun(interaction: ChatInputCommandInteraction) {
		...
		const district: string = interaction.options.getString(
			"district",
			true,
		);
		...
		return SentryHelper.tracer(
			...
			async (span: any) => {
				...
				const managers: string[] | null = await Sentry.startSpan(
					...
					async (childSpan) => {
						Sentry.logger.info("Fetching managers from database", {
							district,
						});

						try {
							const districtManagers: string[] =
								await GetManagersFromDistrict(district, span);

							childSpan.setStatus({ code: 1 });
							Sentry.logger.info("Fetched managers from DB", {
								district,
								count: Array.isArray(districtManagers)
									? districtManagers.length
									: 0,
							});
							...
							await interaction.editReply({
								content:
									"An error occurred while fetching the managers for the specified district.",
							});
				...
				if (managers === null) {
					span.setAttribute(
						"command.status",
						"failed_database_fetch",
					);
					Sentry.logger.info(
						"Command failed: database fetch returned null",
						{
							district,
						},
					);
