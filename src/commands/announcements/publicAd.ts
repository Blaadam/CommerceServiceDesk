...
const BUTTONS = [
	...
	new ButtonBuilder()
		...
		.setStyle(ButtonStyle.Link)
		.setURL(
			"https://docs.google.com/presentation/d/1VgXrj63ATSy4E3FMsk_QcMjmowom-_NlmkJ4PM1np0g/edit?usp=sharing",
		),
	new ButtonBuilder()
	...
	public async chatInputRun(interaction: ChatInputCommandInteraction) {
		const adContainer = new ContainerBuilder()
			...
			.addTextDisplayComponents((textDisplay) =>
				textDisplay.setContent(`## ${AD_HEADER}`),
			);

		const files = [
			new AttachmentBuilder("/app/data/noyra_office_fs.png", {
				name: "noyra_office_fs.png",
			}),
		];
		...
		for (const part of MESSAGE.trim().split("__SPLIT__")) {
			...
			files[fileIndex] &&
				adContainer.addMediaGalleryComponents((gallery) =>
					gallery.addItems((item) =>
						item.setURL(`attachment://${files[fileIndex].name}`),
					),
				);
