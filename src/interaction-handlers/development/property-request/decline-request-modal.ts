		...
		SentryHelper.tracer(
			...
			async (span) => {
				...
				if (!message) {
					...
					span.setAttribute(
						"interaction.response",
						"Original message not found in the upload channel.",
					);
					span.setStatus({ code: 2 });

					return interaction.editReply({
						content: `Original message not found in the upload channel.\n<#${channel.id}>`,
					});
				}
				...
				const landPermit: string =
					embed.fields.find((field) => field.name === "Land Permit")
						?.value || "unknown";
				...
				Sentry.metrics.count(
					"property.development.request.declined",
					1,
					{
						attributes: {
